...
class DACS(UDADecorator):
    ...
    def forward_train(self,
        ...
        if not self.source_only:
            ...
            pseudo_label, pseudo_weight, ema_softmax = self.get_pseudo_label_and_weight(
                ema_logits)
            del ema_logits
            ...
            if self.ecap:
                for i in range(batch_size):
                    ...
                    if res is not None: # extra_augment can be None if some of the memory-banks are empty. They should be populated in the beginning of training.
                        extra_img, extra_label, mask_extra, _, _ = res
        ...
        if (not self.source_only) and self.ecap:
            time_start = time.time_ns()
            ...
            for i in range(batch_size):
                ...
                self.ea_online_ds.manage_threads(target_img[i], pseudo_label[i], ema_softmax[i], file_name_temp)#, target_gt_semantic_seg[i][0])
