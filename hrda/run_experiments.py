...
# The code is based on: https://github.com/lhoyer/HRDA
# ---------------------------------------------------------------
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help='Path temporary dir ECAP memory bank is stored.',
    )
    parser.add_argument(
        '--work_dirs',
        type=str,
        default=None,
        help='Path to directory where all the logs and results are saved.',
    )
    parser.add_argument(
    ...
    # temp = {"tmp_dir": args.tmp_dir}
    # Training with Predefined Config
    if args.config is not None:
        cfg = Config.fromfile(args.config)
        # cfg = cfg._merge_a_into_b(temp, cfg)
        ...
        if args.work_dirs is None:
            ...
            child_cfg = {
                '_base_': args.config.replace('configs', '../..'),
                'name': unique_name,
                'work_dir': os.path.join(args.work_dirs, exp_name, unique_name),
                'git_rev': get_git_hash()
            } 
    ...
    if args.exp is not None:
        ...
        for i, cfg in enumerate(cfgs):
            ...
            if args.work_dirs is None:
                ...
                cfg['work_dir'] = os.path.join(args.work_dirs, exp_name, cfg['name'])
    ...
    if args.machine == 'local':
        for i, cfg in enumerate(cfgs):
            ...
            train.main([config_files[i], "--options", f"uda.ecap_config.tmp_dir={args.tmp_dir},{child_cfg['work_dir']}"])
